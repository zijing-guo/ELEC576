...
def main():
    ...
    with tf.name_scope('W_conv1'):
        var_summaries(W_conv1)
    with tf.name_scope('bias_conv1'):
        var_summaries(b_conv1)
    with tf.name_scope('h_conv1'):
        var_summaries(h_conv1)
    with tf.name_scope('h_pool1'):
        var_summaries(h_pool1)
    ...
    with tf.name_scope('W_conv2'):
        var_summaries(W_conv2)
    with tf.name_scope('bias_conv2'):
        var_summaries(b_conv2)
    with tf.name_scope('h_conv2'):
        var_summaries(h_conv2)
    with tf.name_scope('h_pool2'):
        var_summaries(h_pool2)
    ...
    # setup training
    cross_entropy = tf.reduce_mean(-tf.reduce_sum(y_ * tf.log(y), reduction_indices = [1])) # what's reduce mean?
    ...
    for i in range(max_step):
        ...
        if i % 1100 == 0 or i == max_step:
            ...
            test_ary = accuracy.eval(feed_dict={
                x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1.0})
            val_ary = accuracy.eval(feed_dict={
                x: mnist.validation.images, y_: mnist.validation.labels, keep_prob: 1.0})
            i_e = i // 1100
            print("epoch %d, training accuracy %g" % (i_e, test_ary))
            print("epoch %d, validation accuracy %g" % (i_e, val_ary))
